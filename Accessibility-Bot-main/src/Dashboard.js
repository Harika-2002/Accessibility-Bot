...
const translations = {
  en: {
    ...
    logout: "Logout",
    welcomeMessage: "Welcome — message or use voice commands to interact with the bot.",
...
export default function Dashboard({ showSidebar = true }) {
  ...
  }, [zoomLevel, isNightMode, language]); // Fixed: removed t.darkMode and t.lightMode dependencies

  // useEffect(() => {
  //   document.documentElement.style.setProperty("--zoom", `${zoomLevel}%`);
  //   if (srLiveRef.current) {
  //     srLiveRef.current.textContent = `Zoom ${zoomLevel} percent. ${isNightMode ? "Dark mode on." : "Light mode on."}`;
  //   }
  // }, [zoomLevel, isNightMode]);

  useEffect(() => {
    const onDocClick = (e) => {
      if (!e.target.closest(".zoom-dropdown") && !e.target.closest(".zoom-dropdown-content")) {
        setIsZoomDropdownOpen(false);
      }
      if (!e.target.closest(".account-dropdown") && !e.target.closest(".account-dropdown-toggle")) {
  ...
  const handleZoomIn = () => {
    setZoomLevel((z) => clamp(z + 10, 50, 150));
    announceToScreenReader(`Zoomed in to ${clamp(zoomLevel + 10, 50, 150)} percent`);
  };
  
  const handleZoomOut = () => {
    setZoomLevel((z) => clamp(z - 10, 50, 150));
    announceToScreenReader(`Zoomed out to ${clamp(zoomLevel - 10, 50, 150)} percent`);
  };
  
  const toggleNightMode = () => {
    setIsNightMode((s) => !s);
    announceToScreenReader(!isNightMode ? "Dark mode activated" : "Light mode activated");
  };
  ...
  const startListening = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (event.results[0].isFinal) handleNavigationCommand(transcript.toLowerCase());
    };
  ...
  const handleNavigationCommand = (command) => {
    ...
    if (command.includes("go to") || command.includes("open") || command.includes("navigate to")) {
      const site = Object.keys({
        google: 1, youtube: 1, facebook: 1, instagram: 1, twitter: 1, threads: 1, jira: 1, figma: 1, github: 1, vercel: 1,
    ...
    if (command.includes("toggle mode") || command.includes("dark mode") || command.includes("light mode")) {
      toggleNightMode();
      actionTaken = true;
    }
    if (command.includes("zoom in") || command.includes("magnify") || command.includes("increase text")) {
      handleZoomIn();
      actionTaken = true;
    }
    ...
    if (destination) {
      window.speechSynthesis.speak(new SpeechSynthesisUtterance("Navigating to your requested page."));
      navigate(destination);
      return;
    }

    if (!actionTaken) window.speechSynthesis.speak(new SpeechSynthesisUtterance("I did not understand. Please try again."));
    else window.speechSynthesis.speak(new SpeechSynthesisUtterance("Action completed successfully."));
  };
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setChatHistory((prev) => [...prev, { id: Date.now() + 1, sender: "bot", text: "I heard you — working on it." }]);
  ...
  return (
    <div className={`dashboard ${isNightMode ? "night-mode" : ""}`} lang={language} aria-live="polite">
      {/* Header */}
      <header className="dashboard-header" role="banner">
        ...
        <nav className="header-controls" role="navigation" aria-label="Top navigation">
          {/* Home */}
          <div className="nav-item nav-home" role="button" tabIndex={0} aria-current="page" title={t.home}>
            <img src={homeIcon} alt="" className="nav-icon" aria-hidden="true" />
            <span className="nav-label">{t.home}</span>
          </div>

          {/* Help */}
          <div className="nav-item" role="button" tabIndex={0} onClick={() => navigate("/help")} 
               onKeyDown={(e) => handleKeyActivate(e, () => navigate("/help"))} aria-label={t.help}>
            <img src={helpIcon} alt="" className="nav-icon" aria-hidden="true" />
            <span className="nav-label">{t.help}</span>
          </div>

          {/* Zoom */}
          <div className="nav-item zoom-dropdown" role="button" tabIndex={0} 
               aria-haspopup="true" aria-expanded={isZoomDropdownOpen}
               onClick={() => setIsZoomDropdownOpen((s) => !s)}
               onKeyDown={(e) => handleKeyActivate(e, () => setIsZoomDropdownOpen((s) => !s))}>
            <img src={zoomInIcon} alt="" className="nav-icon" aria-hidden="true" />
            <span className="nav-label">{t.textMagnification} ({zoomLevel}%)</span>
            {isZoomDropdownOpen && (
              <div className="zoom-dropdown-content" role="menu">
                <div role="menuitem" tabIndex={0} onClick={handleZoomIn} 
                     onKeyDown={(e) => handleKeyActivate(e, handleZoomIn)}>
                  ➕ {t.zoomIn}
                </div>
                <div role="menuitem" tabIndex={0} onClick={handleZoomOut}
          ...
          <div className="nav-item" role="button" tabIndex={0} 
            ...
            <img src={isNightMode ? nightModeIcon : brightModeIcon} alt="" className="nav-icon" aria-hidden="true" />
          ...
          {/* Account */}
          <div className="nav-item account-dropdown-toggle" role="button" tabIndex={0} 
               aria-haspopup="true" aria-expanded={isAccountDropdownOpen}
               onClick={() => setIsAccountDropdownOpen((s) => !s)}
               onKeyDown={(e) => handleKeyActivate(e, () => setIsAccountDropdownOpen((s) => !s))}>
            ...
            {isAccountDropdownOpen && (
              <div className="account-dropdown" role="menu" aria-label="Account options">
                <div role="menuitem" tabIndex={0} onClick={() => navigate("/view-account")}
                     onKeyDown={(e) => handleKeyActivate(e, () => navigate("/view-account"))}>
                  👤 View Account Info
                </div>
                <div role="menuitem" tabIndex={0} onClick={() => navigate("/change-password")}
                     onKeyDown={(e) => handleKeyActivate(e, () => navigate("/change-password"))}>
                  🔒 Change Password
                </div>
                <div role="menuitem" tabIndex={0} onClick={() => navigate("/delete-account")}
                     onKeyDown={(e) => handleKeyActivate(e, () => navigate("/delete-account"))}>
                  ⚠️ Delete My Account
                </div>
              </div>
            )}
          ...
          {/* Logout */}
          <div className="nav-item logout" role="button" tabIndex={0} 
      ...
      <div className="dashboard-body">
        <aside className={sidebarClass} aria-label="Primary features" aria-hidden={!sidebarOpen}>
          <ul>
            <li role="button" tabIndex={0} onClick={() => navigate("/text-to-speech")}
                onKeyDown={(e) => handleKeyActivate(e, () => navigate("/text-to-speech"))}>
            ...
            <li role="button" tabIndex={0} onClick={() => navigate("/speech-to-text")}
                onKeyDown={(e) => handleKeyActivate(e, () => navigate("/speech-to-text"))}>
              <span className="icon" aria-hidden="true">🎤</span>
              <div className="content">
            ...
            <li role="button" tabIndex={0} onClick={() => window.open("https://accessibility-bot-multilang.vercel.app/", "_blank")}
                onKeyDown={(e) => handleKeyActivate(e, () => window.open("https://accessibility-bot-multilang.vercel.app/", "_blank"))}>
              <span className="icon" aria-hidden="true">🌐</span>
              <div className="content">
            ...
            <li role="button" tabIndex={0} onClick={() => navigate("/image-to-text")}
                onKeyDown={(e) => handleKeyActivate(e, () => navigate("/image-to-text"))}>
            ...
            <li role="button" tabIndex={0} onClick={() => window.open("https://accessibility-bot-text-summarize.vercel.app/", "_blank")}
                onKeyDown={(e) => handleKeyActivate(e, () => window.open("https://accessibility-bot-text-summarize.vercel.app/", "_blank"))}>
              <span className="icon" aria-hidden="true">📝</span>
              <div className="content">
            ...
            <li role="button" tabIndex={0} onClick={() => window.open("https://accessibility-bot-text-paraphrase.vercel.app/", "_blank")}
                onKeyDown={(e) => handleKeyActivate(e, () => window.open("https://accessibility-bot-text-paraphrase.vercel.app/", "_blank"))}>
              <span className="icon" aria-hidden="true">✍️</span>
              <div className="content">
            ...
            <li role="button" tabIndex={0} onClick={() => window.open("https://accessibility-bot-interactive-dictionary.vercel.app/", "_blank")}
                onKeyDown={(e) => handleKeyActivate(e, () => window.open("https://accessibility-bot-interactive-dictionary.vercel.app/", "_blank"))}>
              <span className="icon" aria-hidden="true">📚</span>
              <div className="content">
            ...
            <li role="button" tabIndex={0} onClick={() => navigate("/keyboardshortcuts")}
                onKeyDown={(e) => handleKeyActivate(e, () => navigate("/keyboardshortcuts"))}>
              <span className="icon" aria-hidden="true">⌨️</span>
              <div className="content">
        ...
        <main className="main-chat">
          ...
          <section className="chat-area">
            {chatHistory?.length === 0 && <p className="welcome-text">{t.welcomeMessage}</p>}

            <div className="bot-image-wrapper">
              {/* <img src={botImage} alt="Bot illustration" className="bot-image" /> */}
            </div>

            {chatHistory?.length > 0 && <div className="chat-history" role="log" aria-live="polite" aria-atomic="false">
              {chatHistory.map((m) => (
          ...
          <div className="chat-input-box-container">
            ...
            {isRecording && <p className="sr-recording" role="status" aria-live="assertive">{t.recording}</p>}
          </div>
        </main>
      </div>

      <div ref={srLiveRef} className="visually-hidden" role="status" aria-live="polite" aria-atomic="true" />
    </div>
  );
}
